	...
	return func(c *gin.Context) {
		...
		if err != nil {
			if err == http.ErrNoCookie {
				c.JSON(http.StatusUnauthorized, response.InitResponse(false, "token is required", nil))
			} else {
		...
		if err != nil {
			if err == exception.ErrTokenExpire || err == exception.ErrInvalidToken {
				c.JSON(http.StatusUnauthorized, response.InitResponse(false, err.Error(), nil))
			} else {
